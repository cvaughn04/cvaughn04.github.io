...
function starImages (rating) {
      ...
      for (let i = 0; i < rating; i++) {
            imageText = imageText + "<img src = 'star.png'>";
      }
      return imageText;
}

for (let i = 0; i < reviewers.length; i++) {
      //build table 1 row at a time
      let reviewCode = "";
      
      if (reviewType[i]  == "P") {
            reviewCode = reviewCode + "<table class = 'prime'>";
      } else if (reviewType[i]  == "N") {
            reviewCode = reviewCode + "<table class = 'new'>";     
      } else {
            reviewCode = reviewCode + "<table>";
      }

      reviewCode = reviewCode + "<caption> " + reviewTitles[i] + " </caption><tr><th>By</th><td> " + reviewers[i] + "</td></tr><tr><th>Review Date</th><td>" + reviewDates[i] + "</td></tr><tr><th>Rating</th><td>" + starImages(stars[i]) + "</td></tr><tr><td colspan='2'>" + reviews[i] + "</td></tr></table>"
